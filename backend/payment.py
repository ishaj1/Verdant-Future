...
def project_transfer_funds():
  ...
  # trans_amount = int(float(amount) * 0.95) # for testing purpose
...
def company_transfer_funds():
  ...
  cursor.execute('SELECT payment_id FROM Company WHERE company_username =  %s', (sender_username))
  src = cursor.fetchone()
  payer_id = src['payment_id']
  payee_id = dest['payment_id']
  amount_transferred = amount
...
def company_transfer_response(): 
  ...
  if(action == "accepted"):
    ...
    amount = amount['amount_transferred']
    print("amount:", amount)
    cursor.execute("SELECT credits_transferred FROM Company_Transaction WHERE transaction_name = %s", (transaction_name))
    credits_amount = cursor.fetchone()
    credits_transferred = credits_amount['credits_transferred']
    print("credits_amount:", credits_amount)
    cursor.execute("SELECT payer_id FROM Company_Transaction WHERE transaction_name = %s", (transaction_name))
    source = cursor.fetchone()
    source = source['payer_id']
    cursor.execute("SELECT payee_id FROM Company_Transaction WHERE transaction_name = %s", (transaction_name))
    destination = cursor.fetchone()
    destination = destination['payer_id']
    ...
    sender_username = sender_username['sender_username']
    ...
    receiver_username = receiver_username['receiver_username']
    result = stripe.Charge.create(
      amount= amount,
      currency="usd",
      source= "acct_1P5t9bQSnkzLsREY"
    ...
    print("query3 success")
    
    query4 = "UPDATE Company SET funds_received = funds_received + %s, funds_required = funds_required - %s, total_credits = total_credits - %s WHERE company_username = %s"
    cursor.execute(
      query4,
        (
          amount,
