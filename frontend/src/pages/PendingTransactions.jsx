            ...
            {transactions.map((transaction, index) => (
              <div key={index} className="border-b-2 border-gray-200 py-4">
                <p>
                  ...
                  {transaction.sender_company_name}
                </p>
                <p>
                  <span className="font-semibold">To:</span>{" "}
                  {transaction.receiver_company_name}
                </p>
                <p>
                  <span className="font-semibold">Credits:</span>{" "}
                  {transaction.credits_transferred}
                </p>
                <p>
                  <span className="font-semibold">Amount:</span> $
                  {(transaction.amount_transferred / 100).toFixed(2)}
                </p>
                {transaction.sender_username == auth.username ? (
                  ...
                  <>
                    <div className="mt-3 flex justify-center">
                      <button
                        className="mr-10 p-2 bg-customGreen-200 rounded-sm hover:bg-customGreen-400"
                        onClick={() =>
                          respondToTransaction(
                            transaction.transaction_name,
                            "accepted"
                      ...
                      <button
                        className="p-2 bg-customGreen-200 rounded-sm hover:bg-customGreen-400"
                        onClick={() =>
                          respondToTransaction(
                            transaction.transaction_name,
                            "declined"
